...
if($libAuth->isLoggedin()){
	...
	$board = array('senior', 'sen_dech', 'consenior', 'con_dech', 'fuchsmajor', 'fm_dech', 'fuchsmajor2', 'fm2_dech', 'scriptor', 'scr_dech', 'quaestor', 'quaes_dech', 'jubelsenior', 'jubelsen_dech');
	...
	$fields = array_merge(array('semester'), $board, $ahv, $hv, $warte, $vorort);
	...
	*/
	...
	elseif($action == 'insert'){
		...
		if($count > 0){
			...
			$semesterRow = $libDb->insertRow($fields, $_REQUEST, 'base_semester', array('semester' => $_REQUEST['semester']));
		}
	}
	...
	elseif($action == 'update'){
		...
		$semesterRow = $libDb->updateRow($fields,$_REQUEST, 'base_semester', array('semester' => $_REQUEST['semester']));
	}
	//keine Aktion
	else {
		$stmt = $libDb->prepare('SELECT * FROM base_semester WHERE semester=:semester');
